...
> 이 문서는 "나이스가이 미용실" 프로젝트에서 도출된 개선 사항입니다.
> 적용 대상 리포지토리: `Parandurume-Laps/conduct`
...
- Google Apps Script 배포, reCAPTCHA 설정 등 **플랫폼 특성에 의한 시행착오**
- 고객 vs 관리자 **역할 분리를 초기에 고려하지 않아 재설계**

---

## 2. 변경 제안

### 2-1. 신규 파일: `references/SME-TEMPLATES.md`
...
### 2-2. AGENT-TEAMS.md에 추가할 행

`Project Type → Recommended Team` 테이블에 추가:
...
### 2-3. SKILL.md Phase 1 수정

Intake의 Step 1 — Parse the Request 이후에 추가:
...
# 5. 결과 확인
# AGENTS.md, CLAUDE.md가 자동 재생성됨

# 6. 커밋
git add .
git commit -m "feat: add SME website templates with user stories and deployment checklists"
```
...
| Before | After |
|---|---|
| 비개발자가 "미용실 웹사이트 만들어줘" → 기능 빠뜨림 | 업종 감지 → User Story 자동 제안 → 빠뜨림 없음 |
| 예약 시스템 추가 시 재설계 필요 | 처음부터 고객/관리자 역할 분리 + 리소스 관리 포함 |
| Google Apps Script 배포 시행착오 | 플랫폼별 주의사항 체크리스트로 방지 |
| 개인정보 동의 누락 | 법적 요구사항 체크리스트로 필수 포함 |
